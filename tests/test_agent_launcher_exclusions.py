...
def test_invoke_persists_gemini_exclusion_when_rate_limited(monkeypatch):
    ...
    monkeypatch.setattr(agent_launcher.StateManager, "load_launched_agents", lambda **_kwargs: dict(state))
    monkeypatch.setattr(agent_launcher.StateManager, "save_launched_agents", lambda data: state.update(data))
